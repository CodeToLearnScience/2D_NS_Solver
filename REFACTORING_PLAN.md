...
| 5 🔶 | Inviscid scheme ports. **5a done:** shared face-flux infrastructure (`define_convective_faces`, average-state RHS assembly, primitive differences), LLF 1st-order and LLF 2nd-order MUSCL — each verified against complete legacy chain goldens (`FluxConvDefns → Diss_LLFx → Avg_Conv_Flux1`), bitwise. Both divergent legacy `MaxEigVal` variants preserved. Free-stream-preservation invariant test added. **5b remaining:** Roe (TV/2nd), MOVERS family, KFDS/ECCS/NKFDS variants — required before Phase 6 for the three baseline cases (schemes 7, 24, 30) | 59 tests green incl. 2 scheme-parity chains; ASan/UBSan clean |
